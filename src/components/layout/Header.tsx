    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#0D6938] text-white flex items-center justify-center shadow-md shadow-emerald-900/20 font-bold text-lg tracking-wider border border-emerald-600/30">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-base sm:text-lg font-extrabold tracking-tight text-slate-900 dark:text-white flex items-center gap-1.5">
                DEPTH 15
                <span className="text-xs px-1.5 py-0.5 rounded font-semibold bg-emerald-100 dark:bg-emerald-950/60 text-[#0D6938] dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800">
                  IRL
                </span>
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {rebaseSession.isOpen ? (
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full bg-amber-100 dark:bg-amber-950/70 text-amber-900 dark:text-amber-200 border border-amber-300 dark:border-amber-700 animate-pulse"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-600" />
              <span>Gate {rebaseSession.currentGate} Open</span>
            </button>
          ) : (
            <button
              onClick={onOpenRebase}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-mono rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition"
            title="Copy invite link with group code"
          >
            <Shield className="w-3.5 h-3.5 text-[#0D6938]" />
            <span className="font-semibold">{groupCode}</span>
            {copied ? (
          ...
          <div className="relative flex items-center">
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
              <div
              ...
              <select
                ...
                className="text-xs font-medium bg-transparent text-slate-800 dark:text-slate-200 focus:outline-hidden cursor-pointer pr-1"
              >
                {members.map((m) => (
          ...
          <button
            onClick={onOpenRules}
            className="p-1.5 rounded-lg text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
